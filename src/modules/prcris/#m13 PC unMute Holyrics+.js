...
function moduleLog(message, values) {
    if (!currentModule || !currentModule.settings || currentModule.settings.log !== true) {
        return;
    }
...
function startup(module) {
    ...
    moduleLog('PC unMute v{} | rota={} | wing={} | x32={} | soundcraft={}', [
        moduleVersion,
        getMixerModelLabel(module.settings.digital_mixer_id),
        jsc.wing && typeof jsc.wing.setChannelVolume === 'function' ? (jsc.wing.version || 'publicada') : 'nao carregada',
        jsc.x32 && typeof jsc.x32.setChannelVolume === 'function' ? (jsc.x32.version || 'publicada') : 'nao carregada',
        jsc.soundcraft && typeof jsc.soundcraft.conn === 'function' ? (jsc.soundcraft.version || 'publicada') : 'nao carregada'
    ]);
}
...
function info() {
    return {
        ...
        description: '<html>' +
                    ...
                    '• <span style="background-color: yellow;"><font color="black"><b> ##NEW </b></font></span> Automatically selects WING, X32/M32 or Soundcraft from the receiver type.<br>' +
                    '• Uses only the Community integrations: jsc.wing, jsc.x32 and jsc.soundcraft.<br>' +
                    infoVDDMM,
        ...
        i18n: {
            ...
            description: {
                en: '<html>' +
                    ...
                    '• Automatically selects WING, X32/M32 or Soundcraft from the receiver type.<br>' +
                    '• Uses only the Community integrations: jsc.wing, jsc.x32 and jsc.soundcraft.<br>' +
                    infoVDDMM,
                pt: '<html>' +
                    ...
                    '• <span style="background-color: yellow;"><font color="black"><b> ##NOVO </b></font></span> Seleciona automaticamente WING, X32/M32 ou Soundcraft pelo tipo do receptor.<br>' +
                    '• Usa somente as bibliotecas publicadas na Community: jsc.wing, jsc.x32 e jsc.soundcraft.<br>' +
                    infoVDDMM,
                es: '<html>' +
                    ...
                    '• Selecciona automáticamente WING, X32/M32 o Soundcraft según el tipo de receptor.<br>' +
                    infoVDDMM,
                ru: '<html>' +
                    ...
                    '• Автоматически выбирает WING, X32/M32 или Soundcraft по типу приемника.<br>' +
                    infoVDDMM,
                it: '<html>' +
                    ...
                    '• Seleziona automaticamente WING, X32/M32 o Soundcraft in base al tipo di ricevitore.<br>' +
                    infoVDDMM
            }
        }
    };
...
function settings() {
    return [
        ...
        },
        {
            ...
            description: '<html><hr>' + jsc.i18n('O módulo identifica automaticamente a integração pelo tipo do receptor: Wing, Soundcraft ou OSC para X32/M32.'),
            type: 'receiver',
...
function getMixerModel(receiverID) {
    if (!receiverID) return null;
    var receiverInfo = h.getReceiverInfo(receiverID);
    if (!receiverInfo) throw 'Receptor nao encontrado: ' + receiverID;
    var receiverType = String(receiverInfo.type || '').toLowerCase();
    if (receiverType === 'wing') return 'wing';
    if (receiverType === 'soundcraft') return 'soundcraft';
    if (receiverType === 'osc') return 'x32';
    throw 'Tipo de receptor não suportado para mixer: ' + (receiverType || 'desconhecido');
}

function getMixerModelLabel(receiverID) {
    try {
        var model = getMixerModel(receiverID);
        if (!model) return 'Não configurado';
        return model === 'x32' ? 'X32/M32' : model === 'soundcraft' ? 'Soundcraft' : 'WING';
    } catch (e) {
        return 'Receptor inválido (' + mixerErrorText(e) + ')';
    }
}
...
function normalizeMixerMuteState(value) {
    if (value === null || value === undefined) return null;
    if (value === true || value === 1) return true;
    if (value === false || value === 0) return false;

    var normalized = String(value).toLowerCase();
    if (normalized === '1' || normalized === 'true' || normalized === 'on' || normalized === 'muted') return true;
    if (normalized === '0' || normalized === 'false' || normalized === 'off' || normalized === 'unmuted') return false;
...
function validateMixerReceiver(receiverID, mixerModel) {
    ...
    if (mixerModel === 'wing' && receiverType !== 'wing') {
        throw 'WING exige receptor do tipo Wing; atual=' + (receiverType || 'desconhecido');
    }
...
function getX32Target(receiverID, channel, targetType) {
    ...
    return {
        ...
        toggleMute: function() {
            requireMixerMethod(jsc.x32, 'is' + action + 'Mute', 'jsc.x32');
            requireMixerMethod(jsc.x32, 'set' + action + 'Mute', 'jsc.x32');
            var currentMute = normalizeMixerMuteState(jsc.x32['is' + action + 'Mute'](receiverID, channel));
            if (currentMute === null) throw 'jsc.x32.is' + action + 'Mute não retornou um estado válido.';
            return jsc.x32['set' + action + 'Mute'](receiverID, channel, !currentMute);
        },
        smoothDelayMs: 10
    };
}
...
function getSoundcraftTarget(receiverID, channel, targetType) {
    ...
    return {
        ...
        toggleMute: function() {
            var target = connection();
            requireMixerMethod(target, 'isMute', 'jsc.soundcraft.target');
            var currentMute = normalizeMixerMuteState(target.isMute());
            if (currentMute === null) throw 'jsc.soundcraft.target.isMute não retornou um estado válido.';
            if (currentMute) {
                requireMixerMethod(target, 'unmute', 'jsc.soundcraft.target');
                return target.unmute();
            }
...
function getMixerTarget(receiverID, channel, channelType) {
    if (!receiverID) return null;
...
function unMute(receiverID, channel, channelType) {
    if (!receiverID) {
        return false;
    }

    try {
        var target = getMixerTarget(receiverID, channel, channelType);
        if (!target) {
            return false;
        }
        try {
...
function muteMixer(receiverID, channel, channelType) {
    if (!receiverID) {
        return false;
    }
...
function setVolume(receiverID, channel, volume, channelType) {
    if (!receiverID) {
        return false;
    }
...
function getMute(receiverID, channel, channelType) {
    if (!receiverID) {
        return null;
    }
    ...
    try {
        ...
        return normalizeMixerMuteState(result);
    } catch (e) {
...
function getVolume(receiverID, channel, channelType) {
    if (!receiverID) {
        return null;
    }
...
function toggleMute(receiverID, channel, channelType) {
    if (!receiverID) {
...
function setVolumeGradually(module, receiverID, channel, targetVolume, channelType, durationSeconds, onFirstStep, onComplete) {
    ...
    var initialVolume;
    try {
        target = getMixerTarget(receiverID, channel, channelType);
    ...
    if (initialVolume == null || isNaN(initialVolume)) {
        var directResult = setVolume(receiverID, channel, finalVolume, channelType);
        if (directResult) {
            notifyFirstStep();
            notifyComplete();
        }
    ...
    if (Math.abs(finalVolume - initialVolume) < 0.001) {
        var sameResult = setVolume(receiverID, channel, finalVolume, channelType);
        if (sameResult) {
            notifyFirstStep();
            notifyComplete();
        }
    ...
    if (!target || typeof target.setSmoothVolume !== 'function') {
        var fallbackResult = setVolume(receiverID, channel, finalVolume, channelType);
        if (fallbackResult) {
            notifyFirstStep();
            notifyComplete();
        }
    ...
    try {
        ...
        var failedSmoothFallback = setVolume(receiverID, channel, finalVolume, channelType);
    ...
    module.setTimeout(function() {
        ...
        if (!setVolume(receiverID, channel, finalVolume, channelType)) return;
...
function createSetMediaVolumeContextAction(module, type) {
    return {
        name: jsc.i18n('Definir volume deste item na mesa'),
        icon: 'volume_up',
        types: [type],
        action: function(evt) {
            var item = evt.item || {};
...
function createRemoveMediaVolumeContextAction(type) {
    return {
        name: jsc.i18n('Remover volume individual deste item'),
        icon: 'volume_off',
        types: [type],
        action: function(evt) {
            var item = evt.item || {};
...
function applyConfiguredMediaEnd(module, type, obj) {
    ...
    module.setTimeout(function() {
        ...
        function completeEnd() {
            if (shouldMute) {
                if (muteMixer(receiverID, channel, s.channel_type)) {
        ...
        if (savedData.gradual) {
            ...
            cancelMixerVolumeFade();
            if (setVolume(receiverID, channel, endVolume, s.channel_type)) {
                completeEnd();
            } else {
            ...
            action: (function(currentType) {
                return function(obj) {
                    ...
                    if (volumeData.gradual) {
                        ...
                        if (!setVolumeGradually(module, receiverID, channel, volume, s.channel_type, volumeData.fade_seconds, function() {
                            unMute(receiverID, channel, s.channel_type);
                        })) {
                        ...
                        if (setVolume(receiverID, channel, volume, s.channel_type)) {
                            unMute(receiverID, channel, s.channel_type);
                        } else {
...
function actionVolumeChannel(module) {
    return {
        ...
        action: function(evt) {
            var s = module.settings;
            var volume = getVolume(s.digital_mixer_id, s.mixer_channel, s.channel_type);
...
function actionStatusChannel(module) {
    return {
        ...
        action: function(evt) {
            var s = module.settings;
            toggleMute(s.digital_mixer_id, s.mixer_channel, s.channel_type);
        },
        status: function(evt) {
            if (!module.isEnabled()) {
                return;
            ...
            var s = module.settings;
            var mute = getMute(s.digital_mixer_id, s.mixer_channel, s.channel_type);
            var result = {
